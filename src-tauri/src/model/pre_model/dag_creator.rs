...
use crate::subtensor::pattern::{Pattern, Relation};
...
impl DagCreator <'_>{
    pub fn new<'a>(identifier_mapper: &'a IdentifierMapper) -> DagCreator<'a>{
        let patterns: Vec<&Pattern> = identifier_mapper.getRepresentations()
            .iter()
            .map(|r| (*r).asPattern())
            .collect();
    ...
    fn refreshOverlappingRelations(&mut self, first_node: &u32, second_node: &u32, relation: Relation){
        ...
        if first_pattern_density >= second_pattern_density{
            self.dag.addOverlappingNode(second_node, first_node);
        }

        if first_pattern_density <= second_pattern_density {
            self.dag.addOverlappingNode(first_node, second_node);
        } 
    }
    ...
    fn getRelationedFonts(&mut self, node: &u32) -> HashMap<u32, Relation> {
        let fonts: Vec<u32> = self.dag.getFontNodesIdentifiers();
    ...
    pub fn create(mut self) -> Dag{
        let unorganized_nodes: Vec<u32> = self.dag.getNodesIdentifiers();
        // let unorganized_nodes: Vec<u32> = vec![2, 5, 1, 4, 3]; // FOR TESTING ONLY

        debug_println!("Unorganized nodes: {:?}", &unorganized_nodes);

        let bar = progress_bar::new(unorganized_nodes.len() as u64,"Patterns inserted on DAG");
        ...
        println!("\nNb of fonts found: {}", self.dag.getFontNodes().len());
